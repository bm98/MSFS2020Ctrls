...
//    private const string c_Header = @"\rtf1\ansi\ansicpg437\deff0\deflang9"; // plain ANSI with old IBM codepage, def font use 0, lang is Gen English
    ...
    private string AsUnicode(string text )
    {
      string uc = "";
      foreach( char c in text ) {
        Int32 cNum = Convert.ToInt32( c );
        if (cNum>127)
          uc += $"\\u{Convert.ToInt32( c )}?";
        else
          uc += c;
      }
      return uc;
    ...
    public void WriteTabUC( string input )
    {
      m_rTFtextLine += @"\tab " + AsUnicode( input );
    }

    public void WriteUC( string text )
    {
      m_rTFtextLine += AsUnicode( text );
    }

    public void WriteLnUC( string text )
    {
      m_rTFtextLine += AsUnicode( text );
      WriteLn( );
    }
